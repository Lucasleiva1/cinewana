...
export function App(){
  ...
  const homeRows=useMemo(()=>[
    ...
    ['Películas',remote.movies]
  ] as Array<[string,MediaItem[]]>,[remote.continueWatching,remote.items,remote.movies,remote.recentlyAdded]);
...
function MediaCard({item,openDetail,loadArtwork}:{item:MediaItem;openDetail:(item:MediaItem)=>void;loadArtwork:(id:string)=>Promise<string|null>}){const src=useArtwork(item,loadArtwork);return <button className="media-card" onClick={()=>openDetail(item)}><div className="poster">{src?<img src={src} alt=""/>:<span>{initials(displayTitle(item))}</span>}{item.progressPercent>0&&<i><b style={{width:`${item.progressPercent}%`}}/></i>}</div><strong>{displayTitle(item)}</strong><small>{item.year||'Sin año'}{item.kind==='episode'?` · T${item.seasonNumber||0} E${item.episodeNumber||0}`:''}</small></button>}
...
function SeriesCard({item,openSeries,loadArtwork}:{item:SeriesItem;openSeries:(item:SeriesItem)=>void;loadArtwork:(id:string)=>Promise<string|null>}){const src=useRemoteImage(item.episodeId,item.artworkAvailable,loadArtwork);return <button className="media-card" onClick={()=>openSeries(item)}><div className="poster">{src?<img src={src} alt=""/>:<span>{initials(item.title)}</span>}</div><strong>{item.title}</strong><small>{item.seasons} {item.seasons===1?'temporada':'temporadas'} · {item.episodes} episodios</small></button>}
