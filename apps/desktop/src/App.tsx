...
import {
  ...
  Clapperboard, Copy, Plus, QrCode, Radio, RotateCcw, Search, Settings, ShieldCheck, Smartphone, Star, Tags, Trash2, Tv, UserRound, Users, Wifi, X
} from 'lucide-react';
...
export function App() {
  ...
  /* Sacar de Continuar viendo no borra nada: la marca como terminada y conserva el minuto donde
     quedó, así se puede seguir viendo igual desde la ficha. */
  const dismissFromContinue=async(item:MediaSummary)=>{try{setError(null);setHome(await invoke<HomeDto>('set_media_watched',{mediaId:item.id,watched:true}));await refresh();}catch(cause){setError(String(cause));}};
...
function HomePage({home,hero,heroIndex,setHeroIndex,openDetail,openSeries,openSaga,setFlag,playMedia,category,setCategory,onDismiss}:{home:HomeDto;hero?:MediaSummary;heroIndex:number;setHeroIndex:(n:number)=>void;openDetail:(id:string)=>void;openSeries:(series:SeriesSummary)=>void;openSaga:(saga:SagaSummary)=>void;setFlag:(i:MediaSummary,f:'favorite'|'watchlist')=>void;playMedia:(id:string)=>void;category:string|null;setCategory:(id:string|null)=>void;onDismiss:(i:MediaSummary)=>void}){
  ...
  return <div className="content home-page">
    ...
    {active
      ...
      :<>
        <MediaRow title="Continuar viendo" items={home.continueWatching} openDetail={openDetail} setFlag={setFlag} playMedia={playMedia} onDismiss={onDismiss}/>
...
function MediaRow(props:{title:string;items:MediaSummary[];openDetail:(id:string)=>void;setFlag:(i:MediaSummary,f:'favorite'|'watchlist')=>void;playMedia:(id:string)=>void;onDismiss?:(i:MediaSummary)=>void}){if(!props.items.length)return null;return <section className="media-section"><div className="section-title"><h2>{props.title}</h2><span>{props.items.length}</span></div><CarouselRow label={props.title}>{props.items.map(i=><MediaCard key={i.id} item={i} {...props}/>)}</CarouselRow></section>}
function CatalogPage({title,items,openDetail,setFlag,playMedia,onDismiss}:{title:string;items:MediaSummary[];openDetail:(id:string)=>void;setFlag:(i:MediaSummary,f:'favorite'|'watchlist')=>void;playMedia:(id:string)=>void;onDismiss?:(i:MediaSummary)=>void}){return <div className="content catalog-page"><div className="page-heading"><div><span className="eyebrow">TU BIBLIOTECA</span><h1>{title}</h1></div><span>{items.length} resultados</span></div>{items.length?<div className="card-grid">{items.map(i=><MediaCard key={i.id} item={i} openDetail={openDetail} setFlag={setFlag} playMedia={playMedia} onDismiss={onDismiss}/>)}</div>:<div className="empty-results"><Film/><h2>No hay contenido en esta sección</h2><p>Los elementos aparecerán después del próximo escaneo.</p></div>}</div>}
