...
export function App() {
  ...
  const settingsProps=boot?{boot,scan,updating,updateMessage,updateVersion:availableUpdate?.version,onRescan:rescan,onChoose:chooseFolder,onLogout:logout,onCheckUpdates:checkForUpdates,onInstallUpdate:installAvailableUpdate,onResolveIdentification:resolveIdentification,remote:remoteStatus,remoteBusy,runRemoteAction}:null;
...
function RemoteSettingsPage(props:SettingsProps&{remote:RemoteStatus|null;remoteBusy:boolean;runRemoteAction:(command:string,args?:Record<string,unknown>)=>Promise<void>}){
  ...
  return <div className="cw-shared-settings"><SettingsPage {...settings}/><IdentificationReviewSettings reviews={props.boot.identificationReviews} onResolve={onResolveIdentification}/><div className="content settings-page remote-settings-section"><section className="settings-card remote-settings-card"><div className="settings-icon"><Radio/></div><div className="settings-main">
