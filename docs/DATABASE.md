...
TMDB imports add `metadata_status`, source URL, import/check timestamps, candidate matches, and the cached `metadata.json` path. Local poster and backdrop paths point to the private application cache and are rebased when that complete cache is moved to another Windows installation. CINE WANA does not write sidecar files into media folders; original movie/series roots stay read-only, and metadata JSON files are stored under the application cache by media fingerprint. The disabled Wikipedia importer remains available in code only as a rollback provider.

Scanner identification also stores its source, review state, review reason, and whether the final classification was chosen manually. A manual movie/episode decision is protected from later automatic scans. Lightweight identification manifests are stored under the private application cache by fingerprint and contain titles, hierarchy, description, genres, and cast without creating sidecars or folders in the media root.
