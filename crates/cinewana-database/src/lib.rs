...
impl Database {
    ...
    fn migrate(&self) -> Result<()> {
        ...
        let has_preview = {
            ...
            columns.iter().any(|column| column == "preview_cache_key")
        };
        if !has_preview {
            let conn = self.connection.lock();
            conn.execute(
                "ALTER TABLE media_items ADD COLUMN preview_cache_key TEXT",
                [],
            )?;
            conn.execute(
    ...
    pub fn set_artwork(
        ...
        self.connection.lock().execute(
            "UPDATE media_items SET poster_cache_key=?1,backdrop_cache_key=?2,preview_cache_key=?3,updated_at=?4 WHERE id=?5",
    ...
    pub fn media_detail(&self, id: &str) -> Result<Option<MediaDetail>> {
        let conn = self.connection.lock();
        let summary = conn
            .query_row(MEDIA_SELECT_BY_ID, [id], row_to_summary)
            .optional()?;
    ...
    pub fn set_flag(&self, media_id: &str, flag: &str, value: bool) -> Result<()> {
        ...
        let conn = self.connection.lock();
        conn.execute(
            "INSERT OR IGNORE INTO user_flags(media_item_id,updated_at) VALUES(?1,?2)",
            params![media_id, now],
        )?;
        match flag {
            "favorite" => conn.execute(
                "UPDATE user_flags SET favorite=?1,updated_at=?2 WHERE media_item_id=?3",
                params![value as i64, now, media_id],
            )?,
            "watchlist" => conn.execute(
                "UPDATE user_flags SET in_watchlist=?1,updated_at=?2 WHERE media_item_id=?3",
                params![value as i64, now, media_id],
            )?,
    ...
    pub fn save_progress(&self, media_id: &str, position_ms: i64, duration_ms: i64) -> Result<()> {
        ...
        conn.execute(
            "INSERT INTO watch_history(id,media_item_id,played_at,position_ms) VALUES(?1,?2,?3,?4)",
            params![Uuid::new_v4().to_string(), media_id, now, position_ms],
        )?;
        Ok(())
    }
    ...
    fn preserves_progress_when_file_goes_offline() {
        ...
        assert_eq!(
            db.connection
                .lock()
                .query_row(
                    "SELECT position_ms FROM watch_progress WHERE media_item_id=?1",
                    [id],
                    |r| r.get::<_, i64>(0)
                )
                .unwrap(),
