...
use cinewana_core::{is_supported_video, parse_media_name, MediaTechnical};
...
pub fn discover(root: &Path, recursive: bool) -> Result<Vec<PathBuf>> {
    ...
    let mut videos = WalkDir::new(root)
        ...
        .collect::<Vec<_>>();
    videos.sort_by(|a, b| a.to_string_lossy().to_lowercase().cmp(&b.to_string_lossy().to_lowercase()));
    Ok(videos)
}
...
pub async fn inspect(path: &Path, ffprobe: Option<&Path>) -> Result<DiscoveredFile> {
    ...
    let modified_at = metadata.modified().ok().and_then(|m| m.duration_since(UNIX_EPOCH).ok()).map(|v| v.as_secs() as i64).unwrap_or(0);
    ...
    Ok(DiscoveredFile {
        ...
        file_name: path.file_name().and_then(|s| s.to_str()).unwrap_or_default().to_owned(),
        file_size: metadata.len() as i64,
        modified_at,
        fingerprint: fingerprint(path, metadata.len(), modified_at).unwrap_or_else(|_| format!("{}:{modified_at}", metadata.len())),
...
pub fn parse_ffprobe(value: &Value) -> Result<MediaTechnical> {
    ...
    let duration_ms = format.and_then(|f| f.get("duration")).and_then(Value::as_str).and_then(|s| s.parse::<f64>().ok()).map(|v| (v * 1000.0).round() as i64);
    let transfer = video.and_then(|v| v.get("color_transfer")).and_then(Value::as_str).unwrap_or_default();
    let hdr_type = match transfer { "smpte2084" => Some("HDR10".to_string()), "arib-std-b67" => Some("HLG".to_string()), _ => None };
    Ok(MediaTechnical {
        duration_ms,
        width: video.and_then(|v| v.get("width")).and_then(Value::as_i64),
        height: video.and_then(|v| v.get("height")).and_then(Value::as_i64),
        container: format.and_then(|f| f.get("format_name")).and_then(Value::as_str).map(str::to_owned),
        video_codec: video.and_then(|v| v.get("codec_name")).and_then(Value::as_str).map(str::to_owned),
        audio_codec: audio.and_then(|v| v.get("codec_name")).and_then(Value::as_str).map(str::to_owned),
        hdr_type,
    })
}
...
fn find_external_subtitles(video: &Path) -> Vec<String> {
    let Some(parent) = video.parent() else { return vec![]; };
    let stem = video.file_stem().and_then(|s| s.to_str()).unwrap_or_default().to_lowercase();
    let mut matches = std::fs::read_dir(parent).into_iter().flatten().filter_map(Result::ok).map(|e| e.path()).filter(|candidate| {
        let extension = candidate.extension().and_then(|s| s.to_str()).unwrap_or_default().to_ascii_lowercase();
        let candidate_stem = candidate.file_stem().and_then(|s| s.to_str()).unwrap_or_default().to_lowercase();
        matches!(extension.as_str(), "srt" | "ass" | "ssa" | "vtt") && candidate_stem.starts_with(&stem)
